...
import pandas as pd
K_score = pd.read_csv('Fast_result/score.csv',index_col=0)
K_label = pd.read_csv('Fast_result/true_label.csv',index_col=0)

score = K_score.values
label = K_label.values

y_scores = [x.item() for x in score]
y_true = [x.item() for x in label]
...
def plot_result_auc(args, label, predict, auc):
    ...
    fpr2, tpr2, threshold2 = roc_curve(y_true, y_scores)
    ...
    plt.plot(fpr, tpr, color='darkorange',
             lw=lw, label='REDDA ')
    plt.plot(fpr2, tpr2, color='steelblue',
             lw=lw, label='FastGTN')
    ...
    plt.grid(True)
    plt.savefig(os.path.join(args.saved_path, 'KFC_REDDA_GTN_auc.png'))
    plt.clf()
...
def plot_result_aupr(args, label, predict, aupr):
    ...
    # plt.plot(precision2, recall2, color='steelblue',
    #          lw=lw, label='AUPR Score (area = %0.4f)' % aupr)
    plt.plot(precision, recall, color='darkorange',
             lw=lw, label='REDDA')
    plt.plot(precision2, recall2, color='steelblue',
             lw=lw, label='FastGTN')
    plt.plot([1,0], [0,1], color='grey', lw=lw, linestyle='--')
    plt.xlim([-0.1, 1.1])
    plt.ylim([-0.1, 1.1])
    plt.xlabel('Recall')
